...
if [ -z "$SKIP_FLASHING_SUPPORT" ]; then
  ...
  if [ -e "/media/$USER" ]; then
    ...
    errcho "Warning: could not find media."
    errcho "Will be unable to support flashing mass-storage bootloaders"
  fi

  if [ -n "$media_dir" ]; then
    media_dir_args="-v $media_dir:/media/qmk:z -e USER=qmk"
  fi
fi
