  ...
  #ifdef PLODAH_TAPDANCE_TAPHOLD_MOAR_ENABLE
    void tap_dance_tap_hold_moar_finished(tap_dance_state_t *state, void *user_data) {
      tap_dance_tap_hold_t *tap_hold = (tap_dance_tap_hold_t *)user_data;
      if (state->pressed) {
        if (state->count == 1
          #ifndef PERMISSIVE_HOLD
  ...
  #endif // PLODAH_TAPDANCE_TAPHOLD_MOAR_ENABLE
